...
export default function Category({ products }: CategoryProps) {
  ...
  if (phrase) {filteredProducts = products.filter((p) =>
        ...
        <div className="w-full mx-auto mt-10">
          <div className="bg-white">
            <div className="w-full mx-auto py-6">
              <h1 className="text-3xl xl:text-5xl font-bold text-black text-center">
                Top Product Collection
                <br /> for Your Office
              </h1>
            </div>
            <div className="items-center justify-center mx-auto flex">
              <div className="grid grid-cols-3 md:flex">
                {categoriesNames.map((categoryName) => (
                  <button
                    key={categoryName}
                    className={`text-lg mr-2 mb-3 lg:mr-4 border-4 border-gray-100 h-10 w-24 rounded-xl bg-gray-100 text-black capitalize ${
                      category === categoryName
                        ? "bg-sky-500 border-sky-500 font-bold"
                        : ""
                    }`}
